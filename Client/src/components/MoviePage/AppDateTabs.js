...
export default class AppDateTabs extends React.Component {
  ...
  componentDidMount(){
      ...
      for(let i = 0; i<this.state.day1.length; i++){
        day1TabTemp.push(<Col l12><a id={`timing-${this.state.day1[i]}`} href='/seatbooking' onClick={sessionStorage.setItem('seatsKey',this.state.day1[i-1])}>{this.state.day1[i].substring(12,16)}</a></Col>);
      }
